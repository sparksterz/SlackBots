...
How? (Install)
-------------
Node JS, after doing that installing npm is as easy as opening an admin comand prompt and typing in: npm install npm -g
redis/chocolatey To install redis for windows, you'll need to install chocolatey first. I'll leave the install implementation up to which you prefer.  You can install redis afterward with the command choco install redis-64 
Python you can get from here. Important note when installing is to make sure to toggle the setting to add python to your Windows PATH variables. Otherwise your Hubot Slack Adapter install will probably still fail.
Git can be installed in many ways and in fact if you don't have it at all, there is an integrated version as part of the Heroku toolbelt if you're not comfortable installing it. You'll also want git in your PATH variables, but if you want to install it for more than just Heroku, you should install this.
Heroku toolbelt You'll want to create a free account on Heroku if you haven't yet done it. Unlike for npm, this one is not an optional step.
...
Alright, I'll go over the process I went through to make hubot-tableflip. The easiest way is to use the generator-hubot-script. This you can install in your custom script's directory and run it and it will supply you with all the files you should need.
...
Once you have your package.json file looking good, open a command prompt in the root of your custom script folder. At this point you can call npm login and follow the prompts to log in. After you're logged in, you can do npm publish and it's as simple as that.
